...
## 11. Cloudflare edge proxy hardening

A post-release probe showed that the custom domain still forwarded `/api/b2b/overview` and `/api/admin/overview` to the AI Studio origin, so changing `server.ts` alone could not protect the public site. Pull request [#19](https://github.com/ezyflyltd-wq/https-github.com-ezyflyltd-wq-Journey-Expert-Ltd/pull/19) added an edge interception in `functions/api/[[path]].ts`. Once the release is deployed, both private overview paths will return HTTP 503 at the Cloudflare edge and will not reach AI Studio. Health endpoints, public pages, Firebase, Firestore, and the AI assistant route remain unchanged.

The production release must be verified again after this edge change. The relevant protected refs should remain aligned: GitHub `main` and `release`, with Cloudflare Pages consuming the validated release branch.
